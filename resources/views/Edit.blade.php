@extends('Layout')
 
@section('content')
<h2 style="margin-top: 12px;" class="text-center">Edit booking</a></h2>
<br>
 
<form action="{{ route('bookings.update', $booking_info->id) }}" method="POST" name="update_booking">
{{ csrf_field() }}
@method('PATCH')
 
<div class="row">
    <div class="col-md-12">
        <div class="form-group">
            <strong>bookhour</strong>
            <input type="text" name="bookhour" class="form-control" placeholder="Enter bookhour" value="{{ $booking_info->bookhour }}">
            <span class="text-danger">{{ $errors->first('bookhour') }}</span>
        </div>
    </div>
    <div class="col-md-12">
        <div class="form-group">
            <strong>booking Code</strong>
            <input type="text" name="bookdate" class="form-control" placeholder="Enter booking Code" value="{{ $booking_info->bookdate }}">
            <span class="text-danger">{{ $errors->first('bookdate') }}</span>
        </div>
    </div>
    <div class="col-md-12">
        <div class="form-group">
            <strong>booknb</strong>
            <textarea class="form-control" col="4" name="booknb" placeholder="Enter booknb" >{{ $booking_info->booknb }}</textarea>
            <span class="text-danger">{{ $errors->first('booknb') }}</span>
        </div>
    </div>
    <div class="col-md-12">
        <button type="submit" class="btn btn-primary">Submit</button>
    </div>
</div>
 
</form>
@endsection